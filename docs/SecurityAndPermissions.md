...
- **内置特权**：少数模块可使用直接平台访问（如 NetworkMock 的 `NWPathMonitor`），这是已知的技术债务
...
```
Manifest permissions → CapabilityFactory.makeCapabilities()
    → GrantedCapabilities（每个能力为可选，nil = 未授予）
    → 传入 Module.handle(command:capabilities:bridge:)
```

**关键**：未声明的权限不会生成能力实例，模块无法访问对应的平台 API。
...
**迁移**（P1）：旧版本使用明文 `UserDefaults` 存储 API Key，P1 一次性迁移到 Keychain，并保留旧格式的兼容读取一版。
