...
- **编译时契约**：通过 `ModuleContract` 协议约束
- **同进程运行**：直接调用，无序列化开销
- **能力约束**：仍需通过 `GrantedCapabilities` 访问平台 API，但授予更宽松
- **默认授权**：内置模块按声明默认获得其 manifest 声明的权限；第三方模块必须获得用户授权后才获得对应能力
...
- 未声明的权限不会生成能力实例。
- 内置模块（`sourceKind: .builtIn`）按声明默认授予能力。
- 第三方模块（`sourceKind: .thirdParty`）即使声明了权限，也必须通过 `PermissionCenter` 授权；未授权时 capability 为 `nil`。
...
`PermissionCenter` 使用 `@Observable` 宏，UI 可以直接订阅权限变更。权限持久化到 UserDefaults 的 `permissions.granted` key。
...
- **后端**：macOS Keychain（`KeychainBackend`）
- **API**：`setSecret(_:for:)` / `secret(for:)` / `deleteSecret(for:)`
- **命名空间**：`module.<moduleID>.<rawKey>`（Keychain account 字段）
- **安全特性**：
  - Keychain 数据加密存储（`kSecAttrAccessibleAfterFirstUnlock`）
  - 每个模块独立命名空间，模块 A 无法读取模块 B 的密钥
  - 模块删除时可选择清除关联密钥
- **测试后端**：`InMemorySecretStoreBackend` 提供内存实现，不依赖 Keychain

> **命名诚实**：`ModuleSecretStore` 的生产后端是 Keychain，数据加密存储。不存在明文 secret store 或 UserDefaults fallback。测试中使用 `InMemorySecretStoreBackend` 替代 Keychain。

### ModuleCacheNamespace — 缓存存储

- **后端**：UserDefaults（App Group suite）
- **API**：`saveDomainState(_:)` / `loadDomainState()` / `clearDomainState()`
...
- **后端**：UserDefaults（App Group suite）
- **API**：`subscript(rawKey:)` / `get(_:forKey:)` / `set(_:forKey:)`
